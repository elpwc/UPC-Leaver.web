$(function () {
  ...
  $("#btn1").click(function () {
    if (String($("input[name='stuXh']").val()) == "") {
      ...
      $("#tips-message").text(
        "开始申请，正在取回结果，请假天数越多等待时间越久（大概一天1秒(?)），请稍等..."
      );
      $("#btn1").attr("disabled", "true");
      var data = getData();
  ...
  $("#testbtn").click(function () {
    $.ajax({
      ...
      //dataType: 'json',
      data: {
        data: '{"app_id":"458","node_id":"","form_data":{"1259":{"User_4":"韦润乾","User_6":"地球科学与技术学院","User_8":"1901040412","User_10":"勘查技术与工程(测井)","User_14":"17865329621","User_55":"壮族","User_60":"勘查(测井)1904","Input_16":"打车","Input_18":"17776231863","Input_22":"购买生活用品","Input_26":"吾悅","Input_28":"李田所","Input_30":"11451419198","Calendar_20":"2021-11-28T16:00:00.000Z","Checkbox_34":[{"value":"1","name":"本人严格按照请假外出要求做好疫情防控工作，一卡通/身份证确保本人使用，所到地点、交通方式、返校时间等均与请假所填写信息一致，在外出期间随时与辅导员保持联系，注重人身安全、财产安全，若发生事故，责任自负。"}]}},"userview":1}',
        agent_uid: "",
      ...
      headers: {
        ...
        Cookie:
          "vjuid=168336; vjvd=cf1a1051c2723709009386cecdefa1a7; vt=153712641",
    ...
    if (text != "") {
      ...
      var data = {
        "comment-name": String($("input[name='comment-name']").val())
          .replace(/<script/gi, "<scribble")
          .replace(/<\/script>/gi, "</scribble>")
          .replace(/<style/gi, "<steel")
          .replace(/<\/style>/gi, "</steel>"),
  ...
  $("#btn4").click(function () {
    ...
    $("input[name='stuBj']").val(
      senkou_[1] +
        getGrade(String($("input[name='stuXh']").val())) +
        "0" +
        String(getRadom(1, 5))
    );
...
function getouttime() {
  return (
    String(getRadom(7, 22)).padStart(2, "0") +
    ":" +
    String(getRadom(0, 59)).padStart(2, "0")
  );
}
...
function getData() {
  data = {
    stuXh: String($("input[name='stuXh']").val()).replace(
      /<script/gi,
      "<scribble"
    ),
    ...
    //'stuStartTime': String($("#out_time").val()),   //每日外出时间
