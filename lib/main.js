$(function () {
  ...
  $("#btn2").click(function () {
    ...
    if (text != "") {
      ...
      var data = {
        'comment-name': String($("input[name='comment-name']").val()).replace("<script", "<scribble"),
        'comment-text': String($("input[name='comment-text']").val()).replace("<script", "<scribble")
      };
      $.ajax({
        url: './comment.php',
        type: 'post',
