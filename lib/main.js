$(function () {
    ...
    if (String($("input[name='stuXh']").val()) == "") {
      ...
      $.ajax({
        ...
        success: function (res) {
          $("#tips-message").text('申请完成，学校接口返回的结果如下:');
  ...
  $("#btn2").click(function () {
    ...
    if (text != "") {
      ...
      var data = {
        'comment-name': String($("input[name='comment-name']").val()).replace("<script", "<scribble").replace("</script>", "</scribble>").replace("<style", "<steel").replace("</style>", "</steel>"),
        'comment-text': String($("input[name='comment-text']").val()).replace("<script", "<scribble").replace("</script>", "</scribble>").replace("<style", "<steel").replace("</style>", "</steel>")
      };
...
function getData() {
  data = {
    'stuXh': String($("input[name='stuXh']").val()).replace("<script", "<scribble"),
